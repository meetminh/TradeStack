...
async fn execute_block_inner(
    ...
    match &block.attributes {
                ...
                match type_.as_str() {
                    ...
                    "inverse_volatility" => {
                        execute_inverse_volatility_children(
                            children,
                            window_of_trading_days.unwrap_or(20), // Default to 20 days if not specified
                            pool,
                            execution_date,
                            parent_weight,
                        )
                    ...
                    "market_cap" => {
                        execute_market_cap_children(children, pool, execution_date, parent_weight)
                            .await
                    }
        ...
        Attributes::Filter { .. } => {
            // Your JSON doesn't use filters, but keeping for completeness
            Ok(Vec::new())
        }
    }
}

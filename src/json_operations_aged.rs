use crate::block::filter::{apply_filter, FilterConfig, SelectConfig, SortConfig};
use crate::models::{Condition, ConditionValue, Node};
...
fn validate_node_with_depth(node: &Node, depth: usize) -> Result<(), ValidationError> {
    ...
    match node {
        ...
        Node::Filter {
            ...
            validate_weight(*weight)?;
            validate_filter_fields(universe, sort, select)?;
            children
                .iter()
                .try_for_each(|child| validate_node_with_depth(child, depth + 1))?;
        }
...
fn validate_condition(condition: &Condition) -> Result<(), ValidationError> {
    ...
    // Modify the value validation section to handle both static and dynamic values
    match &condition.value {
        ConditionValue::Static(value) => {
            // Existing value range validations for static values
            match condition.function.as_str() {
                "cumulative_return" => {
                    validate_value_range("cumulative_return", *value as f32, -100.0, 1000.0)?;
                }
                "price_std_dev" | "returns_std_dev" => {
                    validate_value_range(&condition.function, *value as f32, 0.0, f32::INFINITY)?;
                }
                "rsi" | "sma" | "ema" => {
                    ...
                    validate_value_range_uint(&condition.function, *value as u32, 1, 1000)?;
                }
                _ => {}
            }
        }
        ConditionValue::Dynamic { function, params } => {
            // Validate dynamic function
            let valid_functions = [
                "current_price",
                "sma",
                "ema",
            ...
            if !valid_functions.contains(&function.as_str()) {
                return Err(ValidationError::UnknownFunction(function.clone()));
            }
                ...
                "current_price" => {
                    if params.len() != 1 {
                        return Err(ValidationError::InvalidParameters {
                            ...
                            message: format!("Expected 1 parameter (ticker), got {}", params.len()),
                        });
                    }
                    validate_ticker(&params[0])?;
                }
                _ => {
                    return Err(ValidationError::InvalidParameters {
                        function: function.clone(),
                        message: "Unsupported dynamic function".to_string(),
                    });
                }
            }
        }
...
fn validate_filter_config(config: &FilterConfig) -> Result<(), ValidationError> {
    // Validate universe is not empty
    if config.universe.is_empty() {
        return Err(ValidationError::InvalidFilterConfig(
    ...
    if !valid_functions.contains(&config.sort.function.as_str()) {
        return Err(ValidationError::UnknownFunction(
            config.sort.function.clone(),
        ));
    }

    // Validate sort order
    if !["ASC", "DESC"].contains(&config.sort.order.as_str()) {
        return Err(ValidationError::InvalidFilterConfig(format!(
            "Invalid sort order: {}",
            config.sort.order
        )));
    }

    // Validate select direction
    if !["TOP", "BOTTOM"].contains(&config.select.direction.as_str()) {
        return Err(ValidationError::InvalidFilterConfig(format!(
            "Invalid select direction: {}",
            config.select.direction
        )));
    }

    // Validate count and weights
    ...
    if config.select.count as usize > config.universe.len() {
        return Err(ValidationError::InvalidFilterConfig(format!(
            "Count ({}) exceeds universe size ({})",
            config.select.count,
            config.universe.len()
        )));
    }
...
fn validate_filter_fields(
    ...
    // Your existing filter validation logic from validate_filter_config

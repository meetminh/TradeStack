...
function main()
    ...
    local symbols
    try
        @info "Fetching NASDAQ symbols from URL..."
        url = "https://r2.datahub.io/clt98kj4f0005ia08b4a7ergo/main/raw/data/nasdaq-listed.csv"
        response = HTTP.get(url)
        data = String(response.body)
        nasdaq_df = CSV.read(IOBuffer(data), DataFrame)
        # Handle missing values and convert to String
        symbols = String.(coalesce.(nasdaq_df.Symbol, ""))  # Convert missing to empty string
        symbols = filter(!isempty, symbols)  # Remove empty strings
    catch e
        @warn "Failed to fetch symbols from URL, falling back to local CSV" exception=e
        
        # Fallback to local CSV
        try
            @info "Reading symbols from local CSV..."
            # Use joinpath to create proper file path
            local_csv = joinpath(@__DIR__, "flatui__dataFri Dec 27 2024.csv")
            nasdaq_df = CSV.read(local_csv, DataFrame)
            symbols = String.(coalesce.(nasdaq_df.Symbol, ""))  # Convert missing to empty string
            symbols = filter(!isempty, symbols)  # Remove empty strings
        catch e
            @error "Failed to read local CSV file" exception=e
            # Print current directory and file existence for debugging
            @error "Current directory: $(pwd())"
            @error "File exists: $(isfile(local_csv))"
            return
        end
    end
